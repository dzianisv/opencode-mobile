...
const ALLOWED_PERSISTED_KEYS = new Map<string, string>([
  ...
  ["`${ACCESS_PREFIX}${id}`", "OIDC access token, per connection"],
  ["`${ACCESS_PREFIX}${id}.${i}`", "OIDC access token chunk"],
  ["`${REFRESH_PREFIX}${id}`", "OIDC refresh token, per connection"],
  ["`${REFRESH_PREFIX}${id}.${i}`", "OIDC refresh token chunk"],
  ["`${EXPIRES_PREFIX}${id}`", "OIDC access-token expiry, per connection"],
